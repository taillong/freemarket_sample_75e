...
$(function(){
  function appendFileField(index){
    let html = `<div class="js-file__group" data-index="${index}">
                  <input class="sell-form__image__input__box__field js-file" type="file" name="item[images_attributes][${index}][src]" id="item_images_attributes_${index}_src">
                </div>
                `
  ...
  $('.sell-form__image__input__box').on('change', '.js-file',function(){
    ...
    if (fileIndex.length != 0){
      $('.sell-form__image__input__box').attr("for", `item_images_attributes_${fileIndex[0]}_src`);
